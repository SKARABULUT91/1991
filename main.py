...
async def bulk_unfollow(req: BulkUnfollowRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
// Botun veri göndereceği kapı (Endpoint)
app.post('/api/bot-data', async (req, res) => {
  const { url, durum, bot_id } = req.body;
  const apiKey = req.headers['x-api-key'];

  // 1. Güvenlik Kontrolü: Yabancılar veri gönderemesin
  if (apiKey !== 'KODCUM_SECURE_KEY_2026') {
    return res.status(403).json({ message: "Yetkisiz bot erişimi!" });
  }

  console.log(`🤖 ${bot_id} botundan veri geldi: ${url}`);

  // 2. Veritabanına Yazma (Supabase Entegrasyonu)
  // Bu kısım verinin dashboard ekranında görünmesini sağlar
  try {
    const { data, error } = await supabase
      .from('logs') // Supabase'deki tablo adın neyse o (örn: logs veya bot_data)
      .insert([{ source: bot_id, target_url: url, status: durum, created_at: new Date() }]);

    if (error) throw error;
    res.status(200).json({ status: "Başarılı", message: "Veri DB'ye kaydedildi." });
  } catch (err) {
